...
#!/usr/bin/env python3
"""
Add HDFCBANK stock with historical data from 2023 to present
"""
...
# HDFCBANK Token (from Angel One Scrip Master)
HDFCBANK_TOKEN = "1333"
HDFCBANK_EXCHANGE = "NSE"
...
def main():
    logger.info("=" * 60)
    logger.info("🏦 ADDING HDFCBANK STOCK DATA (2023-2026)")
    logger.info("=" * 60)
    ...
    # Check if HDFCBANK exists
    cursor.execute('SELECT id FROM "Stock" WHERE symbol = %s', ('HDFCBANK',))
    result = cursor.fetchone()
    
    if result:
        stock_id = result[0]
        logger.info(f"✅ HDFCBANK already exists with ID: {stock_id}")
    else:
        # Add HDFCBANK stock
        cursor.execute('''
        ...
        conn.commit()
        logger.info(f"✅ Added HDFCBANK stock with ID: {stock_id}")
    ...
    data = fetch_data_in_chunks(smartApi, HDFCBANK_TOKEN, HDFCBANK_EXCHANGE, start_date, end_date)
    ...
    if data:
        ...
        try:
            ...
            conn.commit()
            logger.info(f"✅ Inserted {len(records)} records for HDFCBANK")
        except Exception as e:
    ...
    conn.close()
    logger.info("\n" + "=" * 60)
    logger.info("🎉 HDFCBANK DATA ADDED SUCCESSFULLY!")
    logger.info("=" * 60)
