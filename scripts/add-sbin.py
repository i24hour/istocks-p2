...
#!/usr/bin/env python3
"""
Add SBIN stock with historical data (from Jan 2023)
"""
...
def main():
    logger.info("=" * 60)
    logger.info("🏦 ADDING SBIN STOCK DATA")
    logger.info("=" * 60)
    ...
    # Check if SBIN exists
    cursor.execute('SELECT id FROM "Stock" WHERE symbol = %s', ('SBIN',))
    result = cursor.fetchone()
    
    if result:
        stock_id = result[0]
        logger.info(f"✅ SBIN already exists with ID: {stock_id}")
    else:
        # Add SBIN stock
        cursor.execute('''
        ...
        conn.commit()
        logger.info(f"✅ Added SBIN stock with ID: {stock_id}")
    ...
    # Resume from Feb 24, 2024 (where previous fetch stopped)
    start_date = datetime(2024, 2, 24)
    end_date = datetime.now()
    
    logger.info(f"\n📅 Fetching data: {start_date.date()} to {end_date.date()}")
    
    data = fetch_data_in_chunks(smartApi, SBIN_TOKEN, SBIN_EXCHANGE, start_date, end_date)
    ...
    if data:
        ...
        try:
            ...
            conn.commit()
            logger.info(f"✅ Inserted {len(records)} records for SBIN")
        except Exception as e:
    ...
    conn.close()
    logger.info("\n" + "=" * 60)
    logger.info("🎉 SBIN DATA ADDED SUCCESSFULLY!")
    logger.info("=" * 60)
