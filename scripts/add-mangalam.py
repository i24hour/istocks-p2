...
#!/usr/bin/env python3
"""
Add Mangalam Industrial Finance (MIFL) stock with historical data (from Jan 2023)
"""
...
# Mangalam Industrial Finance Token
MIFL_TOKEN = "537800"
MIFL_EXCHANGE = "BSE"
...
def main():
    logger.info("=" * 60)
    logger.info("🏭 ADDING MANGALAM INDUSTRIAL FINANCE (MIFL)")
    logger.info("=" * 60)
    ...
    # Check if MIFL exists
    cursor.execute('SELECT id FROM "Stock" WHERE symbol = %s', ('MIFL',))
    result = cursor.fetchone()
    
    if result:
        stock_id = result[0]
        logger.info(f"✅ MIFL already exists with ID: {stock_id}")
    else:
        # Add MIFL stock
        cursor.execute('''
            ...
            RETURNING id
        ''', ('MIFL', 'Mangalam Industrial Finance Ltd', 'BSE'))
        stock_id = cursor.fetchone()[0]
        conn.commit()
        logger.info(f"✅ Added MIFL stock with ID: {stock_id}")
    ...
    data = fetch_data_in_chunks(smartApi, MIFL_TOKEN, MIFL_EXCHANGE, start_date, end_date)
    ...
    if data:
        ...
        try:
            ...
            conn.commit()
            logger.info(f"✅ Inserted {len(records)} records for MIFL")
        except Exception as e:
    ...
    conn.close()
    logger.info("\n" + "=" * 60)
    logger.info("🎉 MIFL DATA ADDED SUCCESSFULLY!")
    logger.info("=" * 60)
