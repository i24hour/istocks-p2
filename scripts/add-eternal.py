...
#!/usr/bin/env python3
"""
Add ETERNAL stock (Zomato) with historical data from 2023 to present
"""
...
# ETERNAL Token (Zomato - renamed to ETERNAL)
ETERNAL_TOKEN = "5097"
ETERNAL_EXCHANGE = "NSE"
...
def main():
    logger.info("=" * 60)
    logger.info("🍕 ADDING ETERNAL STOCK DATA (2023-2026)")
    logger.info("=" * 60)
    ...
    # Check if ETERNAL exists
    cursor.execute('SELECT id FROM "Stock" WHERE symbol = %s', ('ETERNAL',))
    result = cursor.fetchone()
    
    if result:
        stock_id = result[0]
        logger.info(f"✅ ETERNAL already exists with ID: {stock_id}")
    else:
        # Add ETERNAL stock
        cursor.execute('''
            ...
            RETURNING id
        ''', ('ETERNAL', 'Eternal (Zomato)', 'NSE'))
        stock_id = cursor.fetchone()[0]
        conn.commit()
        logger.info(f"✅ Added ETERNAL stock with ID: {stock_id}")
    ...
    # Fetch data from 2023-01-01 to today
    start_date = datetime(2023, 1, 1)
    end_date = datetime.now()
    
    logger.info(f"\n📅 Fetching data: {start_date.date()} to {end_date.date()}")
    
    data = fetch_data_in_chunks(smartApi, ETERNAL_TOKEN, ETERNAL_EXCHANGE, start_date, end_date)
    ...
    if data:
        ...
        try:
            ...
            conn.commit()
            logger.info(f"✅ Inserted {len(records)} records for ETERNAL")
        except Exception as e:
    ...
    conn.close()
    logger.info("\n" + "=" * 60)
    logger.info("🎉 ETERNAL DATA ADDED SUCCESSFULLY!")
    logger.info("=" * 60)
